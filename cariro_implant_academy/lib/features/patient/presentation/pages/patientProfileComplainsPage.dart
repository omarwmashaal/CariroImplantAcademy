    ...
    return Column(
      children: [
        ...
        BlocBuilder<ComplainsBloc, ComplainsBloc_States>(
          buildWhen: (previous, current) => current is ComplainsBloc_LoadingDataSuccessState,
                  ...
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        FormTextValueWidget(
                            secondaryInfo: true, smallFont: true, text: DateFormat("dd-MM-yyyy hh:mm a").format(DateTime.now()).toString()),
                      ],
                    ),
                  ),
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        FormTextValueWidget(
                            secondaryInfo: true, smallFont: true, text: treatments.isEmpty ? "" : treatments.first.supervisor?.name ?? ""),
                      ],
                    ),
                  ),
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        FormTextValueWidget(
                            secondaryInfo: true, smallFont: true, text: treatments.isEmpty ? "" : treatments.first.operator?.name ?? ""),
                      ],
                    ),
                  ),
                  Expanded(
                    child: CIA_DropDownSearchBasicIdName<LoadUsersEnum>(
                      onClear: () {
                        newComplain.mentionedDoctorId = null;
                      },
        ...
        BlocConsumer<ComplainsBloc, ComplainsBloc_States>(
          ...
          builder: (context, state) {
                                                    ...
                                                    CIA_SecondaryButton(
                                                      ...
                                                      onTab: () async {
                                                        CIA_ShowPopUp(
                                                          context: context,
                                                          onSave: () =>
                                                              bloc.add(ComplainsBloc_UpdateComplainNotesEvent(complainId: e.id!, notes: e.notes!)),
            ...
            else if (state is ComplainsBloc_LoadingDataState) return LoadingWidget();
          ...
          listener: (context, state) {
            if (state is ComplainsBloc_ProcessingDataSuccessState) {
              ...
              CustomLoader.hide();
            else if (state is ComplainsBloc_ProcessingDataState) CustomLoader.show(context);
          },
        )
      ],
    );
