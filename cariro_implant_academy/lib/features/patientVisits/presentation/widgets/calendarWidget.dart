...
import '../../../../presentation/patients/bloc/createOrViewPatientBloc_States.dart';
                  ...
                  CIA_DropDownSearchClean(
                    asyncItems: () async {
                      var result = await sl<GetAvailableRoomsUseCase>()(GetAvailableRoomsParams(
                        from: CIA_DateConverters.fromDateTimeToBackend(newVisit.from!),
                        to: CIA_DateConverters.fromDateTimeToBackend(newVisit.to!),
                      ));
                      return result.fold(
                          (l) => [],
                          (r) => r
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CalendarBloc, CalendarBloc_States>(
      listener: (context, state) {
        if (state is CalendarBloc_LoadedRoomsSuccessfully) rooms = state.rooms;
          ...
          if(widget.onNewVisit!=null)
            widget.onNewVisit!(newVisit);
        }
