...
class GetPatientProstheticTreatmentDiagnosticUseCase extends UseCases<ProstheticTreatmentEntity, int> {
  ...
  Future<Either<Failure, ProstheticTreatmentEntity>> call(int id) async {
    return await prostheticRepository.getPatientProstheticTreatmentDiagnostic(id).then((value) => value.fold(
          ...
          (r) {
            r.prostheticDiagnostic_DiagnosticImpression = (r.prostheticDiagnostic_DiagnosticImpression ?? [])
                .map((e) => DiagnosticImpressionEntity(
                      id: e.id,
                ...
                .toList();
            r.prostheticDiagnostic_Bite = (r.prostheticDiagnostic_Bite ?? [])
                .map((e) => BiteEntity(
                      id: e.id,
                      patientId: e.patientId,
                      operator: e.operator,
                ...
                .toList();
            r.prostheticDiagnostic_ScanAppliance = (r.prostheticDiagnostic_ScanAppliance ?? [])
                .map((e) => ScanApplianceEntity(
                      id: e.id,
                      patientId: e.patientId,
                      operator: e.operator,
                      date: e.date,
