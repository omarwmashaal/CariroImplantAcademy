...
class GetTodaysReceiptUsecase extends UseCases<ReceiptEntity, int> {
  final ReceiptRepository receiptRepository;

  GetTodaysReceiptUsecase({required this.receiptRepository});

  @override
  Future<Either<Failure, ReceiptEntity>> call(int patientId) async {
    return await receiptRepository.getTodaysReceipt(patientId: patientId).then((value) => value.fold(
          (l) => Left(l..message = "Get Today's Receipt: ${l.message ?? ""}"),
          (r) => Right(r),
        ));
  }
}
