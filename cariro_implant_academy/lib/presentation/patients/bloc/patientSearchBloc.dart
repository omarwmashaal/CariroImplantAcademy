...
import 'package:cariro_implant_academy/presentation/patients/bloc/patientSeachBlocEvents.dart';
  ...
  PatientSearchBloc({required this.searchUseCase}) : super(LoadingPatientSearchState()) {
    on<PatientSearchEvent>(
      (event, emit) async {
        ...
        final result = await searchUseCase(PatientSearchParams(filter: this._filter, query: event.query, myPatients: false));
        result.fold(
          (l) {
