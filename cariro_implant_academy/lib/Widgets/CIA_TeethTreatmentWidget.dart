...
import 'CIA_MedicalAbsrobPointerWidget.dart';
...
class _getXController extends GetxController {
  ...
  static RxInt availableTacsNumber = 0.obs;
}
...
class _CIA_TeethTreatmentPlanWidgetState extends State<CIA_TeethTreatmentPlanWidget> {
  ...
  _updateTeethStatus(List<int> teeth, String status) {
    for (int tooth in teeth) {
      ...
      if (currentTooth == null) {
        currentTooth = new TreatmentPlanSubModel(tooth: tooth, patientId: widget.patientId);
        ...
        return FocusTraversalGroup(
          ...
          child: Column(
            children: [
                            ...
                            if (tempScrews != (surgicalTreatmentModel.guidedBoneRegenerationCutByScrewsNumber ?? 0)) {
                              if (((surgicalTreatmentModel.guidedBoneRegenerationCutByScrewsNumber ?? 0)) > tempScrews) {
                                await CIA_ShowPopUpYesNo(
                                    title: "Consume ${((surgicalTreatmentModel.guidedBoneRegenerationCutByScrewsNumber ?? 0)) - tempScrews} Screws?",
                                    context: context,
                                    onSave: () async {
              ...
              Obx(() => CIA_MedicalAbsrobPointerWidget(
                    child: Visibility(
  ...
  _buildTeethWidgets() {
    ...
    for (var model in models) {
      returnValue.add(CIA_MedicalAbsrobPointerWidget(
        child: new _ToothWidget(
