            ...
            child: Column(
              ...
              children: [
                Expanded(
                    child: Row(
                  children: [
                    Expanded(child: CIA_DateTimeTextFormField(label: "Date From", controller: TextEditingController(),onChange:(value) => from = value,)),
                    SizedBox(
                      width: 10,
                    ),
                    Expanded(child: CIA_DateTimeTextFormField(label: "Date To", controller: TextEditingController(),onChange:(value) => to = value,)),
                    SizedBox(
                      width: 10,
                    ),
                    Expanded(child: CIA_DropDownSearch(label: "Category",asyncItems: () => SettingsAPI.GetIncomeCategories(),onSelect: (value) => catId=value.id,))
                    ,SizedBox(
                      width: 10,
                    ),
                    Expanded(child: CIA_DropDownSearch(label: "Payment Methods",asyncItems: () => SettingsAPI.GetPaymentMethods(),onSelect: (value) => paymentMethodId=value.id,))
                  ],
                )),
                CIA_PrimaryButton(
                  label: "Filter",
                          ...
                          children: [
                            ...
                            Expanded(
                              child: CIA_Table(
                                ...
                                loadFunction: () async {
                                  return await dataSource.loadData(id: widget.i_dataSource!.models[value-1]!.patientId!, receiptId: widget.i_dataSource!.models[value-1]!.receiptID!);
                                },
                              ),
                            ),
                          ],
